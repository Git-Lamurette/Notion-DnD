...
Some code from an old project I stole that does the similar parsing I want to perform here
...
def build_title(content, layout):
    ...
    label = QLabel(f"{content}")
    label.setWordWrap(True)
    label.setStyleSheet("color: rgb(170, 170, 0);" "font: 14pt Segoe UI;")
    layout.addRow(label)
    build_line(layout)
...
def parse_creature(userin, creatures, main_win_ui):
    for creature in creatures:
        ...
        if userin.lower() == creaturelow.lower():
            ...
            build_single_labels(
                (
                    f"{creature_obj.size} {creature_obj.type.capitalize()} , {creature_obj.alignment.capitalize()}"
                ),
                main_win_ui.formLayout_12,
            )
            ...
            for armor in creature_obj.armor_class:
                if "spell" in armor:
                    ...
                    parsed_armor.append(
                        f"{armor['type'].capitalize()} {armor['value']}"
                    )
            ...
            parsed_speed = [
                f"{Movement}: {Distance}"
                for Movement, Distance in creature_obj.speed.items()
            ]
            ...
            build_labels(
                f"Proficiency Bonus :",
                (f"{creature_obj.proficiency_bonus}"),
                main_win_ui.formLayout_4,
            )
            ...
            if saving_line:
                ...
                build_labels(
                    "Prof Saving Throw :", saving_line, main_win_ui.formLayout_5
                )
            if skill_line:
            ...
            if parsed_dam_vul := list(creature_obj.damage_vulnerabilities):
                ...
                build_labels(
                    f"Vulnerabilities :", parsed_dam_vul, main_win_ui.formLayout_5
                )
            ...
            if parsed_dam_imun := list(creature_obj.damage_immunities):
                ...
                build_labels(
                    f"Damage Immunities :", parsed_dam_imun, main_win_ui.formLayout_5
                )
            ...
            if parsed_con_imun := [
                ...
                build_labels(
                    f"Condition Immunities :", parsed_con_imun, main_win_ui.formLayout_5
                )
            ...
            build_labels(
                f"STR :",
                (
                    f"{creature_obj.strength} ({ability_modifier(creature_obj.strength)})"
                ),
                main_win_ui.formLayout_2,
            )
            build_labels(
                f"DEX :",
                (
                    f"{creature_obj.dexterity} ({ability_modifier(creature_obj.dexterity)})"
                ),
                main_win_ui.formLayout_2,
            )
            build_labels(
                f"CON :",
                (
                    f"{creature_obj.constitution} ({ability_modifier(creature_obj.constitution)})"
                ),
                main_win_ui.formLayout_2,
            )
            build_labels(
                f"INT :",
                (
                    f"{creature_obj.intelligence} ({ability_modifier(creature_obj.intelligence)})"
                ),
                main_win_ui.formLayout_2,
            )
            build_labels(
                f"WIS :",
                (f"{creature_obj.wisdom} ({ability_modifier(creature_obj.wisdom)})"),
                main_win_ui.formLayout_2,
            )
            build_labels(
                f"CHA :",
                (
                    f"{creature_obj.charisma} ({ability_modifier(creature_obj.charisma)})"
                ),
                main_win_ui.formLayout_2,
            )
            ...
            parsed_senses = [
                ...
                for key, value in creature_obj.senses.items()
            ]
            ...
            # ===============
            build_labels(
                "Language(s) :", creature_obj.languages, main_win_ui.formLayout_3
            )
            ...
            for x in creature_obj.special_abilities or []:
                if x["name"].startswith("Spellcasting"):
                    ...
                    parsed_slots = [
                        f"Level {level} - {slots}"
                        for level, slots in x["spellcasting"]["slots"].items()
                    ]
                    parsed_slots = "\n".join(parsed_slots)
                    spell_slots = parsed_slots
                    build_labels(
                        "Spell Slot(s) :", spell_slots, main_win_ui.formLayout_9
                    )
            ...
            for x in creature_obj.special_abilities or []:
                if x["name"].startswith("Spellcasting"):
                    ...
                    for level, spells in spells_by_level.items():
                        spells = " - ".join(spells)
                        build_labels(
                            (f"Level : {level}"), spells, main_win_ui.formLayout_8
                        )
                ...
                for x in creature_obj.special_abilities:
                    ...
                    if x["name"] != ("Spellcasting"):
                        if first_check == False:
                            ...
                            first_check = True
                        build_labels(
                            f'{x['name']} :', x["desc"], main_win_ui.formLayout_11
                        )
